...
def load_student_meta_csv(path):
    ...
    date_birth_parser = lambda dt : datetime.strptime(dt, "%Y-%m-%d %H:%M:%S.%f") if dt else None
    premium_pupil_parser = lambda x : int(float(x)) if x else None 
    ...
    with open(path, "r") as csv_file:
        ...
        for row in reader:
            try:
                user_id = int(row[0])
                gender = int(row[1])
                dob = date_birth_parser(row[2])
                age = None
                if dob:
                    age = today_date.year - dob.year - ((today_date.month, today_date.day) < (dob.month, dob.day))
                premium_pupil = premium_pupil_parser(row[3])
                student_data[user_id] = [gender, age, premium_pupil]
            except:
                raise Exception(f"Parsing error at {row}")
    return student_data


def load_student_meta(root_dir="/data"):
    path = os.path.join(root_dir, "student_meta.csv")
    return load_student_meta_csv(path)


def load_question_meta_csv(path):
    if not os.path.exists(path):
        raise Exception("The specified path {} does not exist.".format(path))
    # Initialize the data.
    question_data = {}
    ...
    with open(path, "r") as csv_file:
        ...
        for row in reader:
            try:
                ques_id = int(row[0])
                question_data[ques_id] = json.loads(row[1])
            except:
                raise Exception(f"Parsing error at {row}")
...
def load_question_meta(root_dir="/data"):
    ...
    return load_question_meta_csv(path)
